...
export default async function CategoryPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8 sm:px-5 sm:py-12"><section className="card rounded-[1.6rem] p-6 sm:rounded-[2rem] sm:p-8 md:p-12"><p className="kicker">Collection</p><h1 className="mt-3 text-3xl font-black sm:text-4xl md:text-6xl">{categoryName(locale, c.slug, c.name)}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-white/55 sm:text-base">{categoryText(locale, c.slug, c.text)}</p></section>{items.length ? <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-9 sm:gap-5 md:grid-cols-3 xl:grid-cols-4">{items.map(p=><ProductCard key={p.slug} p={p}/>)}</div> : <div className="card mt-6 rounded-[1.6rem] p-8 text-center text-white/55">Nog geen Shopify-producten in deze categorie.</div>}</main>
}
