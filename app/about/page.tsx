...
export default async function AboutPage() {
  ...
  const values = [
    { icon: PackageCheck, title: content['about.value1.title'], text: content['about.value1.text'] },
    { icon: ShieldCheck, title: content['about.value2.title'], text: content['about.value2.text'] },
    { icon: Truck, title: content['about.value3.title'], text: content['about.value3.text'] },
  ]
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10 sm:px-5 md:py-14">
    <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(111,125,100,.18),transparent_38%),linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.02))] p-6 md:p-10">
      <p className="text-xs font-black uppercase tracking-[.35em] text-[#b7c8ad]">{content['about.kicker']}</p>
      <h1 className="mt-4 max-w-3xl text-4xl font-black tracking-tight md:text-6xl">{content['about.title']}</h1>
      <div className="mt-5 grid max-w-3xl gap-4 text-base leading-7 text-white/62">
        {paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
      </div>
      <div className="mt-8 flex flex-col gap-3 sm:flex-row">
        <Link href="/shop" className="rounded-full bg-white px-6 py-3 text-sm font-black text-black transition hover:bg-[#dfe8d8]">{content['about.primaryCta']}</Link>
        <Link href="/contact" className="rounded-full border border-white/15 px-6 py-3 text-sm font-black text-white transition hover:bg-white/10">{content['about.secondaryCta']}</Link>
      </div>
    </section>
    ...
    <section className="mt-8 grid gap-5 rounded-[2rem] border border-white/10 bg-white/[.035] p-6 md:grid-cols-[.8fr_1.2fr] md:p-8">
      <div>
        ...
        <h2 className="mt-4 text-2xl font-black">{content['about.bottom.title']}</h2>
      </div>
      <div className="grid gap-4 text-sm leading-7 text-white/58">{splitParagraphs(content['about.bottom.text']).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
    </section>
  </main>
}
