...
import { Bell, Building2, Home, Search, ShieldCheck, Sparkles, UserRound } from 'lucide-react'
...
export default async function AccountPage(){
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-4 py-12 sm:px-5"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="kicker">Mijn Asorta</p><h1 className="mt-3 text-4xl font-black sm:text-6xl">Welkom, {name}.</h1><p className="mt-3 text-white/45">{user.email}</p></div><div className="flex flex-wrap gap-3"><Link href="/privacy-request" className="btn-secondary gap-2"><ShieldCheck size={17}/> Mijn gegevens</Link><Link href="/profile" className="btn-secondary gap-2"><UserRound size={17}/> Profiel</Link></div></div>
