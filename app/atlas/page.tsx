...
import { assertAtlasAccess, hasAtlasPermission, type HousingAtlasPermission } from '@/lib/atlas-auth'
...
export default async function Atlas(){
  ...
  const stats:[HousingAtlasPermission,typeof Users,string,number][]= [
    ...
    ['reports',Flag,'Open meldingen',countReports.count||0],
    ['privacy',ShieldCheck,'Privacyverzoeken',countPrivacy.count||0],
    ['housing',Building2,'Foto’s te keuren',countPhotos.count||0],
  ]
  const tiles:[HousingAtlasPermission,string,React.ReactNode,string,string][]= [
    ...
    ['reports','/atlas/reports',<Flag key="reports"/>,'Rapportages','Onderzoek meldingen en blokkeer accounts indien nodig.'],
    ['swaps','/atlas/swaps',<ArrowRightLeft key="swaps"/>,'Ruildossiers','Volg verhuurderstatussen en afgeronde woningruilen.'],
    ['privacy','/atlas/privacy',<ShieldCheck key="privacy"/>,'Privacyverzoeken','Behandel export- en verwijderverzoeken aantoonbaar.'],
  ...
  return <main className="mx-auto min-h-screen max-w-7xl px-4 py-14 sm:px-5"><p className="kicker">Beheeromgeving</p><h1 className="mt-3 text-4xl font-black sm:text-6xl">ASORTA Woningruil Atlas</h1><p className="mt-3 text-white/45">Ingelogd als {staff.displayName}</p><div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">{stats.filter(([permission])=>hasAtlasPermission(staff,permission)).map(([,Icon,label,value])=><div key={label} className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><Icon className="text-[#b8ff5a]" size={20}/><strong className="mt-7 block text-3xl">{value}</strong><span className="text-xs text-white/42">{label}</span></div>)}</div><div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{tiles.filter(([permission])=>hasAtlasPermission(staff,permission)).map(([,href,icon,title,text])=><Tile key={href} href={href} icon={icon} title={title} text={text}/>)}</div></main>
}
