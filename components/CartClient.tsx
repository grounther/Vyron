  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_390px]">
      <div className="grid gap-4">
        {items.map((item) => (
          <div key={item.slug} className="card grid grid-cols-[92px_1fr] gap-4 rounded-[1.6rem] p-4 md:grid-cols-[120px_1fr]">
            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <button className="rounded-full border border-white/10 px-3 py-1" onClick={() => save(items.map((cartItem) => cartItem.slug === item.slug ? { ...cartItem, qty: Math.max(1, cartItem.qty - 1) } : cartItem))}>-</button>
                <span className="min-w-6 text-center font-black">{item.qty}</span>
                <button className="rounded-full border border-white/10 px-3 py-1" onClick={() => save(items.map((cartItem) => cartItem.slug === item.slug ? { ...cartItem, qty: cartItem.qty + 1 } : cartItem))}>+</button>
                <button className="ml-auto inline-flex items-center gap-2 text-sm text-white/45 hover:text-white" onClick={() => save(items.filter((cartItem) => cartItem.slug !== item.slug))}><Trash2 size={15} /> Remove</button>
              </div>
            </div>
          </div>
        ))}
      ...
      <aside className="card h-fit rounded-[1.8rem] p-6 lg:sticky lg:top-24">
        ...
        <label className="mt-5 grid gap-2 rounded-2xl border border-white/10 bg-black/30 p-4">
          <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.18em] text-white/38"><Mail size={14} /> Recovery email</span>
          ...
          <span className="text-xs leading-5 text-white/40">Hiermee kan ASORTA je cart onthouden en later een reminder sturen als je niet afrondt.</span>
        ...
        <p className="mt-4 flex gap-2 text-xs leading-5 text-white/45"><Lock size={15} /> Volgende fase: Mollie checkout met iDEAL | Wero, PayPal, Cards, Apple Pay en Google Pay.</p>
      </aside>
    </div>
  )
}
