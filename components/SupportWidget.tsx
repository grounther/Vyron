...
import { Headphones, MessageCircle, Send, X, CheckCircle2 } from 'lucide-react'
...
export default function SupportWidget() {
  ...
  async function submitTicket(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Ticket kon niet worden opgeslagen')

      form.reset()
      setState('sent')
    } catch {
      setError('Verzenden lukte niet. Probeer het later opnieuw.')
      setState('error')
    }
  }
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[.045] p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/45">Momenteel offline · laat een bericht achter.</p>
                </div>
              </div>
            </div>
            ...
            {state === 'sent' ? (
              <div className="mt-5 rounded-3xl border border-[#6f7d64]/50 bg-[#6f7d64]/15 p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-white/58">
                  Je support ticket is opgeslagen. Zodra support@asorta.nl actief is koppelen we dit ook aan e-mailnotificaties.
                </p>
              ...
              <form onSubmit={submitTicket} className="mt-5 grid gap-3">
                ...
                <p className="text-center text-xs leading-5 text-white/36">
                  Support wordt voorbereid voor support@asorta.nl. Tickets worden nu veilig opgeslagen in Supabase wanneer je database is gekoppeld.
                </p>
              </form>
            )}
          </div>
