...
export function normalizeShopifyProduct(product: ShopifyProductNode) {
  ...
  return {
    ...
    supplier_name: 'ASORTA fulfillment',
    supplier_url: fields['supplier.url'] || fields.supplier_url || product.onlineStoreUrl || '',
    warehouse: fields['supplier.warehouse'] || fields.warehouse || 'Tracked delivery',
    status: statusFor(product),
    hero_image: images[0] || '/products/asorta-product-fallback.svg',
    images,
    badge: fields['asorta.badge'] || fields['vyron.badge'] || fields.badge || 'New Arrival',
    ...
    specs: unique([
      ...
      product.totalInventory != null && product.totalInventory > 0 ? `Availability: In stock` : '',
    ]),
    tags,
    ...
    shipping_info: fields['asorta.shipping_info'] || fields['vyron.shipping_info'] || 'Je ontvangt tracking zodra je pakket is aangemeld voor verzending.',
    content_ideas: [],
    supplier_notes: fields['supplier.notes'] || fields.notes || 'Neem contact op met support als je vragen hebt over dit product of je bestelling.',
    ...
    supplier_raw: { ...fields, route: 'supplier_fulfillment', shopifyProductId: product.id },
