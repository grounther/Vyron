...
export async function upsertShopifyOrderWebhook(admin: SupabaseClient, payload: ShopifyOrderPayload, topic = 'orders/update') {
  ...
  if (orderDbId && Array.isArray(payload.line_items)) {
    await admin.from('order_items').delete().eq('order_id', orderDbId)
    const rows = payload.line_items.map((line: any) => ({
      order_id: orderDbId,
      product_slug: String(line.product_id || line.sku || line.title || 'shopify-item'),
      product_name: String(line.title || line.name || 'Shopify item'),
  ...
  await admin.from('integration_sync_logs').insert({
    ...
    payload: { shopify_order_id: id, name, tracking },
