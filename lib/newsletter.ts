...
export async function sendResendEmail(input: { to: string | string[]; subject: string; html: string; replyTo?: string; from?: string }) {
  const apiKey = process.env.RESEND_API_KEY
  const from = input.from || process.env.NEWSLETTER_FROM || 'ASORTA <info@asorta.nl>'
